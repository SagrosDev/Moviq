...
1. **Given** a clean checkout with Node.js 24.18.0 LTS
...
- [x] Establish the frontend workspace and locked dependency inputs (AC: 1)
  ...
  - [x] Pin or constrain the approved stack: Node.js 24.18.0 LTS, TypeScript 6.0.x, React 19.2.7, Vite 8.2.x, `@xyflow/react` 12.11.2, and Playwright 1.62.x.
...
- 2026-08-03: Local Node runtime is `26.5.1`; `npm run check:node` intentionally fails unless run on the story-required Node `24.18.0`.
...
- Review verification completed: `node tests/architecture/frontend-boundaries.test.mjs`, `npm run test:unit`, local TypeScript binary `tsc --noEmit`, local Vite build, local Playwright test, and static artifact scan passed. `npm run check:node`, `npm test`, and `npm run build` fail locally by design because this machine uses Node `26.5.1`, not Node `24.18.0`; those scripts now enforce the story-required runtime before continuing.
