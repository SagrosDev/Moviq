...
- [x] Integrate the new evidence into the verification contract (AC: 1, 2, 3, 4)
  - [x] Add any required dependencies deliberately to `Moviqo.Front/package.json` and `package-lock.json`; keep Node 24.18.0 enforcement intact.
...
- Story 1.2 intentionally enforces Node 24.18.0; local machines with Node 26.x may fail normal npm scripts by design. Do not weaken that guard.
...
- 2026-08-03: `npm install axe-core --save-dev` completed; local Node warning preserved because repo enforces Node 24.18.0 and current local Node is 26.5.1.
...
- 2026-08-03: Guarded `npm run typecheck`, `npm run build`, and `npm run test:e2e` are blocked locally by expected Node version guard: expected 24.18.0, received 26.5.1.
...
- Integrated `axe-core`, CI Playwright browser installation/e2e execution, and README verification updates while preserving Node 24.18.0 enforcement and existing API-client/architecture guards.
