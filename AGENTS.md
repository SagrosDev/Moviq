...
- Until Moviqo has a verified Resend sender domain, `moviqo-uat-outbox-drain` must inject `MOVIQO_RESEND_TEST_RECIPIENT` from Secret Manager secret `moviqo-uat-resend-test-recipient`. The secret value is the Resend account email and must never be requested in chat, committed, logged, or added to the Cloud Run service. This routing is allowed only for `synthetic-only` recipients and must be replaced with a verified sender-domain configuration when one becomes available.
