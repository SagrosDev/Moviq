...
- Frontend root: `Moviqo.Front/`
  - `package.json` already enforces Node 26.5.1, Vite/React/TypeScript/Playwright scripts, architecture tests, and static artifact scanning.
...
- Story 1.2 aligned the frontend runtime contract to Node 26.5.1. Keep that enforcement; do not weaken it for local convenience.
...
- Local Node guard preserved: `npm run typecheck` and `npm run build` stop when the runtime does not match the required Node `26.5.1`.
