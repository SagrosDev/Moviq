...
- Frontend root: `Moviqo.Front/`
  - `package.json` already enforces Node 24.18.0, Vite/React/TypeScript/Playwright scripts, architecture tests, and static artifact scanning.
...
- Story 1.2 local Node was 26.5.1, so normal npm scripts intentionally fail unless run on Node 24.18.0. Keep that enforcement; do not weaken it to make local convenience pass.
...
- Local Node guard preserved: `npm run typecheck` and `npm run build` stop because local Node is `26.5.1`, while the project requires `24.18.0`.
